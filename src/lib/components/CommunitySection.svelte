<section id="community">
  <div class="container">
    <div class="card2 community">
      <div class="center pb-1">
        <h2>Community & Support</h2>
      </div>
      <div class="grid three">
        <div class="community__item">
          <h3>Chat with Miners</h3>
          <p>Join the <a href="https://discord" target="_blank" class="link">#mining</a> channel on the Flokicoin Community Discord for real-time updates and tuning tips.</p>
          <a class="btn secondary" href="https://discord" target="_blank">Open Chat</a>
        </div>
        <div class="community__item">
          <h3>Sharenote Basics</h3>
          <p>Discover what Sharenotes are, how they work, and how they help you track your mining performance.</p>
          <a class="btn secondary" href="https://sharenote.xyz" target="_blank">Go to sharenote.xyz</a>
        </div>
        <div class="community__item">
          <h3>Need Support?</h3>
          <p>Join us on Flokicoin Community Discord to get support, share feedback, and connect with other users.</p>
          <a class="btn secondary" href="https://discordd" target="_blank">Join Flokicoin Community</a>
        </div>
      </div>
    </div>
  </div>
</section>

<style>
  .community {
    display: grid;
    gap: 1.6rem;
  }

  .community__item {
    display: grid;
    gap: 0.75rem;
    background: rgba(255, 255, 255, 0.92);
    border-radius: 14px;
    padding: 1.4rem;
    border: 1px solid rgba(188, 206, 255, 0.55);
    box-shadow: 0 12px 28px -22px rgba(12, 46, 94, 0.35);
  }
  .link{font-weight:700}
  .link:hover{text-decoration:underline;}
</style>

